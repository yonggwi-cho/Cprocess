...
- [ ] MC 一般経路が常時材料テーブルを渡す(全 Si 時は高速パス維持)
- [ ] レジストスタック経路がスタック下の実材料を反映
- [ ] 解析注入のカラム毎スクリーニング補正(線量は再正規化しない)
- [ ] 全 Si メッシュで MC/解析ともに変更前とビット一致
- [ ] [W-8] parity 3 チェックが test_implant_materials.cpp で PASS
- [ ] golden flow ①③ PASS(必要なら定数を再計測しコメント付きで更新)
