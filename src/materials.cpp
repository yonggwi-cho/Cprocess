...
//
// [C-1] Range-table extension (1 keV .. 3 MeV, >=15 pts/species), methodology:
// The original 8-point (or 5-point for Sb/In), 10-200 keV core of every
// table below is kept BIT-IDENTICAL to the pre-C-1 values (these anchor the
// permanent Tier-A LSS benchmarks in tests/test_benchmarks.cpp and must not
// move). New points were added below 10 keV and above 200 keV, up to 3 MeV,
// using the SAME Lindhard-Scharff-Schiott (LSS) physics already used by the
// MC engine's electronic-stopping coefficient (`kls_coeff` in
// src/mc_implant.cpp, `3.83e-15*Z1^(7/6)*Z2/(Z1^(2/3)+Z2^(2/3))^1.5/sqrt(M1)`):
// electronic stopping power dE/dx ~ k*sqrt(E), so at high energy the range
// integral dR = dE/(k*sqrt(E)) gives R(E) ~ sqrt(E), i.e. a log-log slope
// (exponent) of exactly 0.5 -- the classical LSS high-energy asymptote.
// Concretely, per species:
//  - High-energy extension (>200 keV, up to 3 MeV): the local log-log slope
//    n = d(ln Rp)/d(ln E) measured from the table's last two existing
//    points (150/200 keV) is linearly relaxed, in ln(E), toward the LSS
//    electronic-stopping asymptote n=0.5 at E=3 MeV, and each new Rp/dRp
//    point is obtained by a stepwise power-law update
//    Rp(E_i) = Rp(E_{i-1}) * (E_i/E_{i-1})^n(E_i) using that (decaying)
//    exponent. This reproduces the well-known LSS/SRIM trend of range
//    growing sub-linearly (indeed -> sqrt(E)) once electronic stopping
//    dominates the nuclear (nuclear-scattering, nearly-linear-in-E) regime
//    that governs the low-energy end of the table.
//  - Low-energy extension (<10 keV, down to 1 keV): the nuclear-stopping
//    regime's local log-log slope (measured between the table's two lowest
//    existing points) is held constant down to 1 keV -- nuclear stopping
//    cross-sections saturate (Lindhard universal function has a broad
//    maximum near reduced energy eps~0.3-1), so range continues to scale
//    close to linearly with E at these very low energies rather than
//    diverging or collapsing.
//  - gamma/beta (Pearson-IV skew/kurtosis) at new energies: NOT independently
//    derived (no rigorous closed-form LSS moment expressions are used
//    elsewhere in this codebase) -- this is an explicit APPROXIMATION.
//    High-energy points are relaxed toward the Gaussian limit
//    (gamma->0, beta->3) as E->3 MeV, consistent with the physical picture
//    that higher-energy implants straggle more symmetrically (less
//    nuclear-scattering-induced skew/leptokurtosis) -- the same qualitative
//    trend already visible descending the pre-existing 200->10 keV rows
//    (gamma and beta both shrink toward the Gaussian point as energy drops...
//    here we extrapolate the *opposite*, energy-increasing, direction using
//    the mirror-image trend). Low-energy points continue the existing
//    10/20 keV two-point linear trend in ln(E), clamped to
//    gamma in [-3,0], beta in [3,12] to avoid unphysical values.
// Values were generated by a small script (not committed; documented here)
// implementing exactly the stepwise procedure above -- reproducible from
// this comment and the retained 10-200 keV anchor rows.
const std::vector<Dopant> kDopants = {
    {"boron", "B", DopType::acceptor, 5, 11.009,
     ...
     {{1, 3.3000 * NM, 3.2401 * NM, 0.000, 3.000},
      {2, 6.6000 * NM, 5.3366 * NM, -0.036, 3.000},
      {5, 16.5000 * NM, 10.3214 * NM, -0.300, 3.000},
      ...
      {300, 738.9879 * NM, 108.3259 * NM, -1.275, 7.251},
      {400, 921.6944 * NM, 120.5730 * NM, -1.116, 6.720},
      {600, 1241.4373 * NM, 141.1309 * NM, -0.891, 5.972},
      {800, 1512.8500 * NM, 158.8334 * NM, -0.732, 5.440},
      {1000, 1750.4838 * NM, 174.7006 * NM, -0.609, 5.028},
      {1500, 2257.9693 * NM, 208.7436 * NM, -0.384, 4.280},
      {2000, 2668.4923 * NM, 238.3860 * NM, -0.225, 3.749},
      {2500, 3015.0460 * NM, 265.1899 * NM, -0.101, 3.337},
      {3000, 3315.6194 * NM, 289.9660 * NM, -0.000, 3.000}},
    ...
    {"phosphorus", "P", DopType::donor, 15, 30.974,
     ...
     2.45e23, 0.62, /*fi*/ 1.0,
     {{1, 1.5798 * NM, 0.8954 * NM, 0.000, 3.000},
      {2, 3.0467 * NM, 1.6629 * NM, -0.068, 3.000},
      {5, 7.2593 * NM, 3.7692 * NM, -0.200, 3.000},
      ...
      {300, 382.4139 * NM, 107.7998 * NM, -0.765, 4.701},
      {400, 500.1377 * NM, 133.8894 * NM, -0.670, 4.488},
      {600, 714.2361 * NM, 179.4061 * NM, -0.535, 4.189},
      {800, 899.7380 * NM, 217.9893 * NM, -0.439, 3.976},
      {1000, 1063.2938 * NM, 251.7598 * NM, -0.365, 3.811},
      {1500, 1416.0228 * NM, 323.8332 * NM, -0.230, 3.512},
      {2000, 1697.5449 * NM, 382.2266 * NM, -0.135, 3.299},
      {2500, 1930.4734 * NM, 431.6186 * NM, -0.061, 3.135},
      {3000, 2127.9852 * NM, 474.5472 * NM, -0.000, 3.000}},
    ...
    {"arsenic", "As", DopType::donor, 33, 74.922,
     ...
     1.3e23, 0.66, /*fi*/ 0.4,
     {{1, 1.3310 * NM, 0.6233 * NM, -0.034, 3.000},
      {2, 2.3662 * NM, 1.0908 * NM, -0.084, 3.000},
      {5, 5.0625 * NM, 2.2857 * NM, -0.150, 3.000},
      ...
      {300, 158.2016 * NM, 49.7247 * NM, -0.595, 4.105},
      {400, 201.2670 * NM, 60.7252 * NM, -0.521, 3.967},
      {600, 277.7997 * NM, 79.6926 * NM, -0.416, 3.773},
      {800, 343.2594 * NM, 95.6955 * NM, -0.342, 3.635},
      {1000, 400.6999 * NM, 109.6929 * NM, -0.284, 3.527},
      {1500, 523.8059 * NM, 139.5029 * NM, -0.179, 3.333},
      {2000, 622.7458 * NM, 163.8234 * NM, -0.105, 3.195},
      {2500, 705.5294 * NM, 184.5669 * NM, -0.047, 3.088},
      {3000, 776.6368 * NM, 202.7520 * NM, -0.000, 3.000}},
     /*seg_m0,seg_e*/ 10.0, 0.0, /*seg_h0,seg_he*/ 1.0e5, 2.0,
    ...
    {"antimony", "Sb", DopType::donor, 51, 120.90,
     ...
     3.8e21, 0.56, /*fi*/ 0.1,
     {{1, 1.5240 * NM, 0.5080 * NM, 0.000, 3.000},
      {2, 2.6012 * NM, 0.8671 * NM, -0.054, 3.000},
      {5, 5.2732 * NM, 1.7577 * NM, -0.137, 3.000},
      {10, 9 * NM, 3 * NM, -0.20, 3.1}, {30, 21 * NM, 7 * NM, -0.30, 3.3},
      {50, 31 * NM, 10 * NM, -0.35, 3.4}, {100, 53 * NM, 17 * NM, -0.50, 3.7},
      {200, 96 * NM, 29 * NM, -0.60, 4.0},
      {300, 135.8904 * NM, 39.6351 * NM, -0.510, 3.850},
      {400, 171.2330 * NM, 48.8975 * NM, -0.446, 3.744},
      {600, 233.5676 * NM, 64.9793 * NM, -0.357, 3.594},
      {800, 286.6774 * NM, 78.5835 * NM, -0.293, 3.488},
      {1000, 333.2243 * NM, 90.4861 * NM, -0.243, 3.406},
      {1500, 432.8011 * NM, 115.8652 * NM, -0.154, 3.256},
      {2000, 513.0688 * NM, 136.4813 * NM, -0.090, 3.150},
      {2500, 580.5121 * NM, 153.9761 * NM, -0.040, 3.067},
      {3000, 638.7123 * NM, 169.2334 * NM, -0.000, 3.000}},
     /*seg_m0,seg_e*/ 10.0, 0.0, /*seg_h0,seg_he*/ 1.0e5, 2.0,
    ...
    {"indium", "In", DopType::acceptor, 49, 114.82,
     ...
     {{1, 1.6003 * NM, 0.5334 * NM, 0.000, 3.000},
      {2, 2.7312 * NM, 0.9104 * NM, -0.054, 3.000},
      {5, 5.5369 * NM, 1.8456 * NM, -0.137, 3.000},
      ...
      {300, 142.6849 * NM, 41.6168 * NM, -0.510, 3.850},
      {400, 179.7947 * NM, 51.3424 * NM, -0.446, 3.744},
      {600, 245.2460 * NM, 68.2283 * NM, -0.357, 3.594},
      {800, 301.0113 * NM, 82.5127 * NM, -0.293, 3.488},
      {1000, 349.8855 * NM, 95.0105 * NM, -0.243, 3.406},
      {1500, 454.4412 * NM, 121.6584 * NM, -0.154, 3.256},
      {2000, 538.7223 * NM, 143.3054 * NM, -0.090, 3.150},
      {2500, 609.5377 * NM, 161.6749 * NM, -0.040, 3.067},
      {3000, 670.6479 * NM, 177.6951 * NM, -0.000, 3.000}},
     /*seg_m0,seg_e*/ 10.0, 0.0, /*seg_h0,seg_he*/ 1.0e5, 2.0,
    ...
    {"carbon", "C", DopType::neutral, 6, 12.011,
     ...
     {{1, 2.6400 * NM, 2.5921 * NM, 0.000, 3.000},
      {2, 5.2800 * NM, 4.2693 * NM, -0.036, 3.000},
      {5, 13.2000 * NM, 8.2571 * NM, -0.300, 3.000},
      ...
      {300, 591.1903 * NM, 86.6607 * NM, -1.275, 7.251},
      {400, 737.3555 * NM, 96.4584 * NM, -1.116, 6.720},
      {600, 993.1499 * NM, 112.9047 * NM, -0.891, 5.972},
      {800, 1210.2800 * NM, 127.0667 * NM, -0.732, 5.440},
      {1000, 1400.3870 * NM, 139.7605 * NM, -0.609, 5.028},
      {1500, 1806.3754 * NM, 166.9949 * NM, -0.384, 4.280},
      {2000, 2134.7939 * NM, 190.7088 * NM, -0.225, 3.749},
      {2500, 2412.0368 * NM, 212.1519 * NM, -0.101, 3.337},
      {3000, 2652.4955 * NM, 231.9728 * NM, -0.000, 3.000}},
     /*seg_m0,seg_e*/ 10.0, 0.0, /*seg_h0,seg_he*/ 1.0e5, 2.0,
     /*dox0,eox*/ 0.0, 0.0,
    ...
    {"fluorine", "F", DopType::neutral, 9, 18.998,
     ...
     {{1, 2.3100 * NM, 2.2681 * NM, 0.000, 3.000},
      {2, 4.6200 * NM, 3.7356 * NM, -0.036, 3.000},
      {5, 11.5500 * NM, 7.2250 * NM, -0.300, 3.000},
      ...
      {300, 517.2915 * NM, 75.8281 * NM, -1.275, 7.251},
      {400, 645.1861 * NM, 84.4011 * NM, -1.116, 6.720},
      {600, 869.0061 * NM, 98.7916 * NM, -0.891, 5.972},
      {800, 1058.9950 * NM, 111.1834 * NM, -0.732, 5.440},
      {1000, 1225.3386 * NM, 122.2904 * NM, -0.609, 5.028},
      {1500, 1580.5785 * NM, 146.1205 * NM, -0.384, 4.280},
      {2000, 1867.9446 * NM, 166.8702 * NM, -0.225, 3.749},
      {2500, 2110.5322 * NM, 185.6329 * NM, -0.101, 3.337},
      {3000, 2320.9336 * NM, 202.9762 * NM, -0.000, 3.000}},
     /*seg_m0,seg_e*/ 10.0, 0.0, /*seg_h0,seg_he*/ 1.0e5, 2.0,
     /*dox0,eox*/ 0.0, 0.0,
